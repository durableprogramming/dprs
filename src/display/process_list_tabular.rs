...
pub fn render_container_table<B: Backend>(f: &mut Frame, app_state: &mut AppState, area: Rect) {
    ...
    let rows = displayed_containers.iter().map(|container| {
        let cells = vec![
            Cell::from(container.name.clone()).style(Style::default().fg(Color::Green)),
            Cell::from(container.image.clone()).style(Style::default().fg(Color::Cyan)),
            Cell::from(container.status.clone()).style(Style::default().fg(Color::Blue)),
            Cell::from(container.ip_address.clone()).style(Style::default().fg(Color::Magenta)),
            Cell::from(container.ports.clone()).style(Style::default().fg(Color::White)),
        ];
