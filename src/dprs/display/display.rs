...
use tachyonfx::{Duration, EffectManager, CellFilter, Interpolation};
...
pub fn draw<B: Backend>(f: &mut Frame, app_state: &mut AppState, toast_manager: &ToastManager, config: &mut Config, effects: &mut EffectManager<()>, elapsed: std::time::Duration) {
    ...
    if !app_state.is_progress_active() {
        add_container_effects::<B>(f, app_state, effects, container_area, elapsed, config);
    }
...
fn render_status_line<B: Backend>(f: &mut Frame, app_state: &AppState, area: ratatui::layout::Rect, config: &Config) {
    ...
    if let Some(container) = app_state.get_selected_container() {
        status_parts.push(format!("Container: {} ({})", container.name, container.status));
    ...
    if !app_state.search_state.matches.is_empty() {
        ...
        let current_match = app_state.search_state.current_match
            .map(|i| i + 1)
            .unwrap_or(0);
        status_parts.push(format!("Search: {}/{} matches", current_match, matches_count));
    }

    // Visual selection info
    if let Some(selection) = &app_state.visual_selection {
        status_parts.push(format!("Selected: {} containers", selection.selected_indices.len()));
    }
...
fn render_command_line<B: Backend>(f: &mut Frame, app_state: &AppState, area: ratatui::layout::Rect, config: &Config) {
...
fn render_progress_modal<B: Backend>(f: &mut Frame, app_state: &AppState, area: Rect, config: &Config, effects: &mut EffectManager<()>, elapsed: std::time::Duration) {
    let modal_width = 70;
    let modal_height = 10;
    ...
    let effect = fx::ping_pong(fx::hsl_shift(None, Some([0.0, 0.0, 10.0]), (Duration::from_millis(1000), Interpolation::SineInOut))
        .with_filter(filter));
...
fn add_container_effects<B: Backend>(
    f: &mut Frame,
